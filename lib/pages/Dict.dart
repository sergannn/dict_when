...
//String url = 'http://3.u0156265.z8.ru/itmo2020/Student/teletest/index.php?a=1';
String url =
    'https://3ff1-88-201-168-182.ngrok-free.app/?api_id=21433623&api_hash=4862f5339c133e3d738d830c7f4250fc&target_username=me';
//String url = 'https://send-sound.onrender.com';
...
class _MyAppState extends State<Demo> with WidgetsBindingObserver {
  ...
  var sound_url = "https://sound-samples.onrender.com/";
  Codec _codec = Codec.flac;
  ...
  Future<void> _initializeExample() async {
    ...
    //var _shared = SharedPreferences.getInstance();
  }
  ...
  play_start_sound(exit) async {
    ...
    await playerModule.startPlayer(
        fromURI: Uri.parse(sound_url + sound_name!).toString(),
        //codec: codec,
  ...
  send_audio(path) async {
    print("sending audio");
    final _prefs = await SharedPreferences.getInstance();
    var api = _prefs.getString('api_id');
    var hash = _prefs.getString('api_hash');
    ...
    final params = {
      ...
      'api_hash': hash,
      'message': "hello",
      'target_username': _prefs.getString("to_phone")!.isNotEmpty
          ? _prefs.getString("to_phone")
          : 'me',
      'phone': phone
    };
    var uri =
        Uri.https("soundblaster.onrender.com", "/send_audio_message", params);
...
//        contentType: MediaType('audio', 'mp3')));
  ...
  send_msg() async {
    ...
    print(params);
    var uri = Uri.https("soundblaster.onrender.com", "/send_message", params);
  ...
  void dispose() {
    ...
    api_id_Controller.dispose();
    api_hash_Controller.dispose();
    api_phone_Controller.dispose();
...
*/
  ...
  void Function()? onStartRecorderPressed([int details = 1]) {
    print("onstart");
    //play_start_sound(true);
    // Disable the button if the selected codec is not supported
  ...
  get_sounds_on_off() async {
    var prefs = await SharedPreferences.getInstance();
    var mode = prefs.getBool('sounds_mode') ?? false;
    setState(() {
      sounds_on_off = mode;
    });
  }
  ...
  getMode() async {
    var prefs = await SharedPreferences.getInstance();
    var mode = prefs.getBool('record_mode') ?? false;
    setState(() {
      record_mode = mode;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    print("hello guys");
    print(_codec);
    //_codec = setCodec(codec);
    print("_codec");
    //setState(() {});
    var platform = Theme.of(context).platform;
    print(platform == TargetPlatform.iOS ? 'iOS' : 'Android or other');
    if (platform == TargetPlatform.android && _codec != Codec.aacADTS) {
      setCodec(Codec.aacADTS);
    }

    // print("codec changed to pcm16wav");
    ...
    Widget playerSection = Column(
      ...
      children: <Widget>[
        ...
        ElevatedButton(
            ...
            onPressed: () {
              serPath = _path.nonNulls.first;
              print(serPath);
              try {
                Uint8List audioByte;
                String myPath = _path.nonNulls.first;
                ...
                CupertinoButton(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      color: const Color.fromRGBO(170, 159, 80, 1),
                    ),
                  ...
                  CupertinoButton(
                      ...
                      child: Container(
                        ...
                        child: GestureDetector(
                            onLongPress:
                                record_mode ? onStartRecorderPressed() : () {},
                            onLongPressEnd: record_mode
                                ? (details) {
                            ...
                            child: FloatingActionButton.extended(
                                backgroundColor:
                                    Color.fromRGBO(222, 220, 200, 1),
                                foregroundColor:
                                    _isRecording ? Colors.red : Colors.black,
                                heroTag: "btn1",
                                onPressed: record_mode
                                    ? null
                                    : onStartRecorderPressed(),
                                //icon: Icon(Icons.refresh),
                                label:
                                    Text('ЗАПИСЬ') // + record_mode.toString()),
                                )),
                      )),
                  SizedBox(height: 10),
                  GestureDetector(
                      ...
                      child: CupertinoButton(
                          onPressed:
                              record_mode ? null : onStartRecorderPressed(),
                          //onStartRecorderPressed(),
                          child: CircleAvatar(
                              radius: 45,
                              ...
                              child: CircleAvatar(
                                radius: 40,
                                backgroundColor:
                                    Color.fromRGBO(236, 234, 214, 1),
                                foregroundColor: Colors.black,
                                child: Icon(Icons.mic_sharp, size: 60),
                              ))))
                ]),
                Column(children: [
                  CupertinoButton(
                      ...
                      child: Container(
                          width: 150, // Фиксированная ширина
                          child: FloatingActionButton.extended(
                              backgroundColor: Color.fromRGBO(222, 220, 200, 1),
                              foregroundColor: Colors.black,
                              heroTag: "btn2",
                              onPressed: () {
                                serPath = _path.nonNulls.first;
                  ...
                  CupertinoButton(
                      ...
                      child: CircleAvatar(
                        ...
                        foregroundColor: Colors.black,
                        child: Image(
                            image: AssetImage('assets/images/yy-1-1-bg.png')),
                      ))
                ]),
              ],
            ),
